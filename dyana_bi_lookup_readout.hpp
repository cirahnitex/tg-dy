...
namespace dyana {
  class bi_lookup_readout {
    ...
    static std::pair<std::function<dyana::tensor()>, std::unordered_map<std::string, unsigned>>
    create_readout_window(const dyana::embedding_lookup &lookup, const dyana::parameter &readout,
                          const std::unordered_set<std::string> &tokens_involved) {
      if (lookup.real_dict_size() <= SAMPLE_THRESHOLD) {
        ...
        return std::make_pair(
          [&readout, sampled_ori_ids]() { return dyana::tensor(readout).select_cols(sampled_ori_ids); },
          window_readout_dict);
      }
    }
  ...
  public:
    ...
    bi_lookup_readout(unsigned embedding_size, const std::unordered_set<std::string> &l0_tokens,
                      const std::unordered_set<std::string> &l1_tokens) : l0_lookup(embedding_size, l0_tokens),
                                                                          l1_lookup(embedding_size, l1_tokens),
                                                                          l0_readout({embedding_size + 1,
                                                                                      l0_lookup.real_dict_size()}),
                                                                          l1_readout({embedding_size + 1,
                                                                                      l1_lookup.real_dict_size()}) {

    }

    bi_lookup_readout(unsigned embedding_size, const std::unordered_set<std::string> &l0_tokens,
                      const std::unordered_set<std::string> &l1_tokens,
                      std::function<std::vector<float>(const std::string &)> get_l0_init_embedding,
                      std::function<std::vector<float>(const std::string &)> get_l1_init_embedding) : l0_lookup(
      embedding_size, l0_tokens, get_l0_init_embedding), l1_lookup(embedding_size, l1_tokens, get_l1_init_embedding),
                                                                                                      l0_readout(
                                                                                                        {embedding_size +
                                                                                                         1,
                                                                                                         l0_lookup.real_dict_size()}),
    ...
    bi_lookup_readout(unsigned embedding_size,
                      const std::unordered_map<std::string, std::vector<float>> &l0_token_and_embeddings,
                      const std::unordered_map<std::string, std::vector<float>> &l1_token_and_embeddings) : l0_lookup(
      embedding_size, l0_token_and_embeddings), l1_lookup(embedding_size, l1_token_and_embeddings), l0_readout(
      {embedding_size + 1, l0_lookup.real_dict_size()}), l1_readout({embedding_size + 1, l1_lookup.real_dict_size()}) {}

    dyana::tensor lookup(const std::string &l0_token, const std::string &l1_token) {
      return dyana::max(l0_lookup.operator()(l0_token), l1_lookup.operator()(l1_token));
    }
    ...
    std::pair<std::string, std::string> readout(const dyana::tensor &embedding) {
      ...
      auto l0_token = l0_lookup.id_to_token(
        dyana::argmax_index((padded_embedding.transpose() * l0_readout).transpose()));
      auto l1_token = l1_lookup.id_to_token(
        dyana::argmax_index((padded_embedding.transpose() * l1_readout).transpose()));
      return std::make_pair(l0_token, l1_token);
    }

    std::pair<dyana::tensor, dyana::tensor>
    lookup_with_loss_slow(const std::string &l0_token, const std::string &l1_token) {
      const auto embedding = lookup(l0_token, l1_token);
      auto padded_embedding = dyana::concatenate({embedding, dyana::tensor(1)});
      auto l0_loss = dyana::pickneglogsoftmax((padded_embedding.transpose() * l0_readout).transpose(),
                                              l0_lookup.token_to_id(l0_token));
      auto l1_loss = dyana::pickneglogsoftmax((padded_embedding.transpose() * l1_readout).transpose(),
                                              l1_lookup.token_to_id(l1_token));
      return std::make_pair(embedding, l0_loss + l1_loss);
    }

    std::function<std::pair<dyana::tensor, dyana::tensor>(const std::string &l0_token, const std::string &l1_token)>
    create_lookup_with_loss_computer(
      ...
      return [this, l0_windowed_readout_weight, l0_windowed_readout_dict, l1_windowed_readout_weight, l1_windowed_readout_dict](
        const std::string &l0_token, const std::string &l1_token) {
        const auto embedding = lookup(l0_token, l1_token);
        auto padded_embedding = dyana::concatenate({embedding, dyana::tensor(1)});
        auto l0_oracle = l0_windowed_readout_dict->at(l0_token);
        auto l0_loss = dyana::pickneglogsoftmax(
          (padded_embedding.transpose() * l0_windowed_readout_weight()).transpose(), l0_oracle);
        auto l1_oracle = l1_windowed_readout_dict->at(l1_token);
        auto l1_loss = dyana::pickneglogsoftmax(
          (padded_embedding.transpose() * l1_windowed_readout_weight()).transpose(), l1_oracle);
        return std::make_pair(embedding, l0_loss + l1_loss);
      };
    }
    ...
    std::vector<std::pair<std::string, float>> translate_l0_to_l1_slow(const std::string &l0_token, unsigned top_X) {
      ...
      dyana::beam_bucket<entry_type> bucket(top_X, [](const entry_type &a, const entry_type &b) {
