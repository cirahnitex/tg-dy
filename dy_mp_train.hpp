  ...
  namespace dy {
    ...
    public:
      _mp_train_learner(unsigned num_workers, unsigned num_epoches, const std::vector<DATUM> &training_set,
        ...
        if (num_workers <= 1) {
          dynet::mp::run_single_process(this, dy::_trainer(), training_set, dev_set, num_epoches, dev_set.size(),
                                        dev_set.size(), 1);
        } else {
    ...
    void fit(unsigned num_epoches, const std::vector<DATUM> &training_set,
             const std::vector<DATUM> &dev_set, std::function<dy::tensor(const DATUM &)> compute_loss,
             std::function<void(const std::exception &, const DATUM &)> on_error, std::function<void()> save) {
      _mp_train_learner<DATUM>(_num_workers(), num_epoches, training_set, dev_set, compute_loss, on_error, save);
    }
    ...
    void fit(unsigned num_epoches, const std::vector<DATUM> &training_set,
             const std::vector<DATUM> &dev_set, std::function<dy::tensor(const DATUM &)> compute_loss,
             std::function<void(const std::exception &, const DATUM &)> on_error) {
      _mp_train_learner<DATUM>(_num_workers(), num_epoches, training_set, dev_set, compute_loss, on_error, []() {});
    }
    ...
    void fit(unsigned num_epoches, const std::vector<DATUM> &training_set,
      ...
      _mp_train_learner<DATUM>(_num_workers(), num_epoches, training_set, dev_set, compute_loss,
                               [](const std::exception &e, const DATUM &d) {
    ...
    void fit(unsigned num_epoches, const std::vector<DATUM> &training_set,
             std::function<dy::tensor(const DATUM &)> compute_loss,
             std::function<void(const std::exception &, const DATUM &)> on_error, std::function<void()> save) {
      _mp_train_learner<DATUM>(_num_workers(), num_epoches, training_set, std::vector<DATUM>(), compute_loss, on_error,
                               save);
    }
