...
namespace tg {
  namespace dy {
    class bi_lookup_readout {
      ...
      static std::pair<dy::tensor, std::unordered_map<std::string, unsigned>> create_readout_window(const dy::embedding_lookup& lookup, const dy::Parameter& readout, const std::unordered_set<std::string>& tokens_involved) {
        if(lookup.real_dict_size()<=SAMPLE_THRESHOLD) {
          ...
          return std::make_pair(dy::tensor(readout), window_readout_dict);
        }
        else {
          std::unordered_map<std::string, unsigned> window_readout_dict;
          ...
          return std::make_pair(dy::tensor(readout).select_cols(sampled_ori_ids), window_readout_dict);
        }
      }
    public:
      ...
      std::function<std::pair<dy::tensor, dy::tensor>(const std::string& l0_token, const std::string& l1_token)> create_readout_with_loss_computer(const std::unordered_set<std::string>& l0_tokens_involved, const std::unordered_set<std::string>& l1_tokens_involved) {
        auto p0 = create_readout_window(l0_lookup, l0_readout, l0_tokens_involved);
        auto l0_windowed_readout_weight = std::make_shared<dy::tensor>(std::move(p0.first));
        ...
        auto l1_windowed_readout_weight = std::make_shared<dy::tensor>(std::move(p1.first));
